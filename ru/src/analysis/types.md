...
Radare2 позволяет организовать папку для хранения описанй типов и включений.
...
Команда `ts` преобразует описание типа C (или, если быть точным, представление SDB) в последовательность `pf`-команд. Смотрите подробнее о [печати структур](../basic_commands/print_modes.md). Команда `tp` использует строку `pf` для печати всех полей типа по текущему смещению/данному адресу:
...
Есть еще один важный аспект использования типов в Radare2 — использование `aht`, что позволяет изменять immediate-значение в коде операции на смещение структуры. Рассмотрим простой пример [RSI]-относительной адресации
...
|  X     |  показать отформатированную последовательность шестнадцатеричных кодов                 |
...
Обратите внимание, что `.cc`-часть является необязательной, и если она не используется, то использоваться соглашение о вызовах по умолчанию для вашей целевой архитектуры. Есть еще один дополнительный необязательный ключ
