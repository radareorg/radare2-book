...
Иногда нам нужно переопределить адрес перехода или вызова, например, в случае замысловатого перемещения, неизвестного radare2, можно изменить значение вручную. Текущую аналитическую информацию о конкретном оп-коде можно проверить с помощью команды `ao`. Используем команду `ahc` для выполнения такого изменения:
