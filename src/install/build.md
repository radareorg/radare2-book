...
- Mark `io.bind` and `asm.mycpu` as static and regenerate files:

  ./configure-plugins --static io.bind asm.mycpu

- Make sure `io.debug` is a shared plugin:

  ./configure-plugins --shared io.debug

- Start from defaults (copy default list) and see the lists:

  rm -f plugins.cfg
  ./configure-plugins --list
...
- Build only a subset of plugins (comma separated):

  meson setup build-small -Dplugins='io.debug,asm.x86' -Duse_dylink=false
  ninja -C build-small

- Change options after setup:

  meson configure builddir -Dwant_threads=false
  ninja -C builddir
